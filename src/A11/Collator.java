...
public class Collator implements Runnable {
   ...
   public void initializeCollatorNode(int port) throws IOException {
      ...
      System.out.println("Node Name : " + this.nodeName + "IP: " + this.collatorIP
            + ", Listenning on : " + this.collatorPORT);
   ...
   public void trafficSummary() {
      ...
      double totalSentPayloads = 0;
      double totalReceivedpayloads = 0;
      
      for (Data data : dataCollection) {
         totalSent += data.sentMessagesCounter;
         totalReceived += data.receivedMessagesCounter;
         totalSentPayloads += data.sumOfSentPayload;
         totalReceivedpayloads += data.sumOfReceivedPayload;
      }
      
      System.out.println("Node name (IP+port)  "  + "Message Sent\t" + "Messsage Received\t" + "Sum(Sent Messages)\t" + "Sum(Received Msg)");
      for (Data data : dataCollection) {
         System.out.println(data.name + "\t\t" + data.sentMessagesCounter + "\t\t" + data.receivedMessagesCounter + "\t\t" + data.sentMessagesCounter + "\t\t" + data.sumOfReceivedPayload);
      }
      System.out.println("---------------------------------------------------------------------------------------------------");
      System.out.println("" + "\t\t" + totalSent + "\t\t" + totalReceived + "\t\t" + totalSentPayloads + "\t\t" + totalReceivedpayloads + "\t\t");
   ...
   public void receiveMessages() throws IOException {
      ...
      /*
       * DataInputStream dis = new DataInputStream(s.getInputStream()); String str = (String)
       * dis.readUTF(); System.out.println("message= " + str);
       */
